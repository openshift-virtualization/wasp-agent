...
export WASP_NAMESPACE="wasp-$(cat /dev/urandom | tr -dc 'a-z0-9' | fold -w 5 | head -n 1)"

echo "wasp-namespace: ${WASP_NAMESPACE}"

if [[ -n "$RANDOM_CR" ]]; then
  export CR_NAME="${WASP_NAMESPACE}"
fi

readonly ARTIFACTS_PATH="${ARTIFACTS}"
readonly BAZEL_CACHE="${BAZEL_CACHE:-http://bazel-cache.kubevirt-prow.svc.cluster.local:8080/kubevirt.io/wasp}"
...
export WASP_NAMESPACE="${WASP_NAMESPACE:-wasp}"
