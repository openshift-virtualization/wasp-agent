...
GO_API_REF_REPO=${GO_API_REF_REPO:-kubevirt/wasp-api}
API_REF_DIR=/tmp/wasp-api
GITHUB_IO_FQDN="https://kubevirt.github.io/wasp-api"
...
git clean -fxd
popd
cp -rf staging/src/kubevirt.io/wasp-api/. "${API_REF_DIR}/"
...
# Generate .gitignore file. We want to keep bazel files in kubevirt/wasp, but not in kubevirt/wasp-api
...
if [ -n "$(git status --porcelain)" ]; then
    git commit --message "wasp-api update by KubeVirt Prow build ${BUILD_ID}"

    # we only push branch changes on main
    if [ "${TARGET_BRANCH}" == "main" ]; then
        git push origin ${TARGET_BRANCH}-local:${TARGET_BRANCH}
        echo "wasp-api updated for ${TARGET_BRANCH}."
    fi
else
    echo "wasp-api hasn't changed."
fi
...
if [ -n "${TARGET_TAG}" ]; then
    ...
    echo "wasp-api updated for tag ${TARGET_TAG}."
fi
